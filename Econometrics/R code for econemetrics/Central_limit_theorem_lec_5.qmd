...
$$\hat{CI}_n = [\bar{X}_n - 1.96 \frac{\hat{\sigma}}{\sqrt{n}}, \bar{X}_n + 1.96 \frac{\hat{\sigma}}{\sqrt{n}}]  $$
When $\sigma$ is replaced by $\hat{\sigma}_n$, the distribution of the standardized sample mean follows the t-distribution with n-1 degrees of freedom.
